            ...
            <div class="flex flex-col">
                ...
                @if(auth()->check() && auth()->id() === $listing->user_id)
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 mt-4">
                        <a href="{{ route('listing.edit', $listing->id) }}"
                           class="px-6 py-3 bg-blue-600 text-white rounded hover:bg-blue-700 transition text-center">
                            Edit listing
                        </a>
                        ...
                        <form method="POST"
                            ...
                            <button type="submit"
                                class="px-6 py-3 bg-red-600 text-white rounded hover:bg-red-700 transition w-full">
                                Delete listing
                            </button>
                        </form>
                    </div>
                ...
                @else
                    <form method="POST" action="{{ route('cart.add', $listing->id) }}"
                        ...
                        <div class="flex items-center border rounded justify-center">
                            <button type="button"
                ...
                <div class="mt-10 border-t pt-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded border text-sm sm:text-base">
                        <div class="font-semibold text-lg">
                            {{ $listing->user->vardas }} {{ $listing->user->pavarde }}
                        </div>

                        @if($listing->user->business_email)
                            <div class="mt-1">Email: {{ $listing->user->business_email }}</div>
                        @endif

                        @if($listing->user->telefonas)
                            <div class="mt-1">Tel: {{ $listing->user->telefonas }}</div>
                        @endif
